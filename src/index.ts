...
export function getPairOrderBook$(pairId: string, limit: number = 40): Observable<{sells: AggregateOrderEntry[], buys: AggregateOrderEntry[]}> {
  let buyOrderBook$ = getPairBuyOrders(pairId).pipe(
  ...
  let sellOrderBook$ = getPairSellOrders(pairId).pipe(
      // aggregate orders per price
      map((fullOrdersMap: Map<number, Order[]>) => {
...
// TODO Add in startAt parameter to allow extracting subsets of results.
export function getPairTradesByTime(pairId: string, startTime: number = 0, endTime: number = 0, limit: number = 1000): Observable<Trade[]> {
  const maxLimit = 1000;
...
// TODO Add in startAt parameter to allow extracting subsets of results.
export function getPairSlicesByTime(pairId: string, startTime: number = 0, endTime: number = 0, limit: number = 1000): Observable<TimeSlice[]> {
  const maxLimit = 1000;
  limit = Utils.roundTo(0, limit); // ensure limit is interger
  limit = Math.min(limit, maxLimit);  // ensure limit is not larger than maxLimit
  startTime = Utils.roundTo(0, startTime);  // ensure startTime is interger
  endTime = Utils.roundTo(0, endTime);  // ensure endTime is integer
  const query = fb.collection("pairs/"+pairId+"/slices")
  ...
  return collectionData(query).pipe(
      map((docs: any[]) => {
          return docs.map((doc: any) => {
              return new TimeSlice(doc.startTime, doc.open, doc.close, doc.high, doc.low, doc.token1Volume, doc.token2Volume, doc.noOfTrades);
          })
      })
  )
}
...
export function getWalletTrades(walletId: string, pairIds: string[], startTime: number = 0, endTime: number = 0, limit: number = 1000): Observable<Trade[]> {
...
export function getWalletFees(walletId: string, pairIds: string[], startTime: number = 0, endTime: number = 0): Observable<{paid: Map<string, number>, earned: Map<string, number>}> {
...
export async function getOrder(orderId: string): Promise<Order | null> {
  ...
  while (orderType && !foundOrder) {
      ...
      if (!foundOrder) {orderType = orderTypes.shift()};
  }
  if (foundOrder) {
      return foundOrder;
  } else {
...
function getPairOrdersMap(orderType: String, pairId: string, sortOrder: Utils.SortOrder): Observable<Map<number, Order[]>> {
...
function getPairOrdersArray(
  ...
  return collectionData(pairOrdersRef).pipe(
      map(orderDocs => {
          return Utils.sortOrdersArray(orderDocs.map(orderDoc => {
...
function calcMarketOrderRequirements(
  ...
  while (
      (
      (order.quantitySpecified && remainingQuantity > 0) || 
      (!order.quantitySpecified && remainingValue > 0)
      ) && 
      currentPrice && 
      (
  ...
  if ((order.quantitySpecified && remainingQuantity > 0)
      || (!order.quantitySpecified && remainingValue > 0)
  ) {
...
export function getWalletTokens(wallet: string): Observable<Map<string, number>> {
  ...
  return collectionData(walletTokensRef, "tokenId").pipe(
      map((docs: any[]) => {
          let tokensMap = new Map<string, number>();
          docs.forEach((doc: any) => {
